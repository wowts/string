...
import { find, gmatch, gsub, format, match, sub } from "./index";
...
test("find with back reference", (t) => {
    const result = find('""', "^((['\"])%2)");
    t.deepEqual(result, [1, 2]);
});

test("find with back reference 2", (t) => {
    const result = find('"test"', `^(['"]).-[^\\]%1`);
    t.deepEqual(result, [1, 6]);
});
...
test("gsub with captures at start", (t) => {
...
test("sub with positive end", (t) => {
    const result = sub("teste", 2, 4);
    t.is(result, "est");
});

test("sub with negative end", (t) => {
    const result = sub("teste", 2, -2);
    t.is(result, "est");
});
